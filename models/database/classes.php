...
class Produto extends Model{
    ...
    public function BuscaProdutoPorEstoque(){
        //$sql = "select * from $this->table order by qtd_estoque asc limit 10";
        $sql = "select * from $this->table where qtd_estoque <= 5 order by qtd_estoque asc";
        $list = connect()->prepare($sql);
    ...
    public function JaExisteCadastroProduto($nome_Prod){        
        $sql = "select * from $this->table where nome = ?" ;
        $list = connect()->prepare($sql);
        $list->bindValue(1, $nome_Prod);
